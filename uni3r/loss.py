...
from uni3r.utils.camera_utils import get_scaled_camera
from uni3r.utils.cuda_splatting import DummyPipeline, render
from uni3r.utils.gaussian_model import GaussianModel
...
class TestLoss(MultiLoss):
    ...
    def __init__(self,
        ...
        if self.pose_align_steps:
            self.pose_embeds = nn.Embedding(num_views * 2 - 1, 9)
    ...
    def compute_loss(self, gt, preds, target_view=None, model=None, pose_deltas=None, evaluate=True):
        ...
        for i in range(len(pred)):
            ...
            gaussians = GaussianModel.from_predictions(pred[i], sh_degree=3)
            # get camera
            # print('view nums:', len(target_view))
            ...
            for j in range(len(target_view_list)):
                ...
                camera = get_scaled_camera(None,
                                           ...
                                           image_shape)
                # render(image and features)
                # TODO: using target_extrinsics for MipNerf360dataset
                rendered_output = render(
                    camera, 
                    gaussians, 
                    self.pipeline, 
        ...
        for idx, img_normalized in enumerate(rendered_images):
            img_single = img_normalized 
            img_permuted = img_single.permute(1, 2, 0) 
            img_numpy = img_permuted.cpu().detach().numpy()
            img_uint8 = (img_numpy * 255).astype(np.uint8)
            pil_image = Image.fromarray(img_uint8)
            
            instance_name = target_view[-1]['label']
            instance_dir = os.path.join(output_dir, str(instance_name))
            os.makedirs(instance_dir, exist_ok=True)
            
            pil_image.save(os.path.join(instance_dir, f'rendered_{idx}.jpg'))
...
def loss_of_one_batch(batch,
    ...
    #     img = view['img']
    #     img_normalized = (img + 1.0) / 2.0  # 反归一化：将 [-1, 1] 映射到 [0, 1]
    #     img_single = img_normalized[0]  # 变成 [3, 256, 256]
    #     img_permuted = img_single.permute(1, 2, 0)  # 变成 [256, 256, 3]
    #     img_numpy = img_permuted.cpu().detach().numpy()
    #     img_uint8 = (img_numpy * 255).astype(np.uint8)
    #     pil_image = Image.fromarray(img_uint8)
    #     pil_image.save(f'output_{idx}.jpg')
